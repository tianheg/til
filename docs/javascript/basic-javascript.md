...
date: 2021-05-04 20:25:51
permalink: /pages/550439/
categories:
  - javascript
tags:
---
...
```js
var myName = "Tianhe Gao";
```

## 转义字符

a literal quote: `"` or `'` 字面意义的引用
...
引号不是字符串中唯一可以被转义的字符。有两个使用转义字符的原因：

1. 为了允许你使用无法打印输出的字符。比如，回车。
2. 为了允许你在字符串中表示多个引号，而不会被 JavaScript 误解。
