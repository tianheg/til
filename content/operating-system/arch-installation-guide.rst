...
除此之外，还有 `archinstall <https://python-archinstall.readthedocs.io/en/latest/index.html>`_
...
#. ``fdisk -l`` 查看硬盘信息：sdb 119.24g 固态，sda 931.51g 机械
#. ``fdisk /dev/sdb`` 固态硬盘分区
...
So mine is VA-API, I supposed to install `libva-mesa-driver <https://wiki.archlinux.org/title/Hardware_video_acceleration#:~:text=VA-API%20on%20Radeon%20HD%202000%20and%20newer%20GPUs>`_.
...
不推荐安装 ``xf86-video-intel``\ ，详见 `Intel graphics - ArchWiki <https://wiki.archlinux.org/title/Intel_graphics#:~:text=Often%20not%20recommended%2C%20see%20note%20below>`_
...
#. https://wiki.archlinux.org/title/Hardware_video_acceleration#:~:text=VDPAU%20on%20Radeon%20R300%20and%20newer%20GPUs
...
将 Google Chrome 的缓存挂载到 /tmp（后因排查 `arch linux system freeze after connecting wifi <https://github.com/tianheg/blog/issues/147>`_ 问题取消挂载）：
