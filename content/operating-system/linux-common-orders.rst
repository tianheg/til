...
#. create a file, which its name is ``YYYY-MM-DD-title.md``
...
#. Add same content in many files
...
#. ``mkdir folder && cd $_``\ : Create folder and open it
#. ``chmod +x script`` make script executable
#. ``wget -T 5 -t 10 -Q 500m -nH -r -E -l inf -k -p -np 'https://www.yidajiabei.xyz'`` download a website `ref <https://bbs.archlinuxcn.org/viewtopic.php?pid=42852#p42852>`_
#. ``curl -V | curl -F "c=@-" "https://fars.ee/"`` https://fars.ee/ 粘贴代码
