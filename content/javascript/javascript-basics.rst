...
在 JavaScript 中使用分号结束声明。\ `是否/在什么时候使用 ``;``\ ？ <https://www.codecademy.com/resources/blog/your-guide-to-semicolons-in-javascript/>`_\ 。\ **数字、字母和 ``$`` 或 ``_``\ 是变量名组成范围**\ ，但是：
...
十进制数有时候也可看成 *floating point numbers* 或 *floats*\ 。
...
它是一种数据类型。只有两个值：\ ``True`` 和 ``False``\ ，且互相排斥。注意：布尔型书写时没有引号，带引号的字符串：\ :raw-html-m2r:`<code class="docutils literal"><span class="pre">"true"&#96;&#96;"false"</span></code>`\ ，不是布尔型，在 JavaScript 中没有特殊意义。
...
:raw-html-m2r:`<mark>对象和数组很像，只不过对象不使用索引存取和修改数据，而是通过属性</mark>`\ 。对象对于以结构化方式存储数据很有用。如果您的对象具有任何非字符串属性，则 JavaScript 会自动将它们作为字符串进行类型转换。
...
#. 你的函数必须总是返回整个记录集合对象。
#. 如果 ``prop`` 不是 ``tracks``\ ，\ ``value`` 非空，更新或将唱片的 ``prop`` 设置成 ``value``\ 。
#. 如果 ``prop`` 是 ``tracks`` 但唱片没有 ``tracks`` 属性，新建一个空数组，添加 ``value``\ 。
#. 如果 ``prop`` 是 ``tracks`` 且 ``value`` 非空，添加 ``value`` 到唱片存在的 ``tracks`` 数组最后。
#. 如果 ``value`` 是空字符串，从唱片中删除 ``prop`` 属性。
